...
def _find_pdf_path(dataset_slug: str, filename: str) -> Path:
    ...
    print("Looking for uploaded PDF at", expected_paths[0])
    for expected in expected_paths:
        if expected.is_file():
            if expected != expected_paths[0]:
                print("Using uploaded PDF at", expected)
            return expected
    ...
    if candidates:
        ...
        _fail(f"Expected one of {expected_paths}, but found multiple PDFs: {available_pdfs}")
